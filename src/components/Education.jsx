        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {educationData.map((edu, index) => (
            <motion.div
              ...
              viewport={{ once: false }}
              className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl hover:scale-[1.02] transition-transform duration-300 ease-in-out border border-gray-200 dark:border-gray-700 h-full"
            >
              <div className="flex items-start">
                <div className="flex-shrink-0 mr-4">
                  {edu.type === "honors" ? (
                    <FaGraduationCap className="text-indigo-600 text-3xl mt-1" />
                  ) : (
                    <FaSchool className="text-indigo-600 text-3xl mt-1" />
                  )}
                </div>
